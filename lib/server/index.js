...
module.exports = function(store, serverOptions, clientOptions) {
  ...
  var middleware = BrowserChannelServer(serverOptions, function(client, connectRequest) {
    ...
    function startBrowserChannel() {
      ...
      doneInitialization(store, stream, connectRequest);
    }
  });
  ...
  wss.on('connection', function (client) {
    ...
    client.on('error', function onError(err) {
      console.error('[racer-highway] web socket error, stopping client:', err);
      client.close();
    });
    ...
    doneInitialization(store, stream, client.upgradeReq);
  ...
  if (serverOptions.session) {
    var emitter = store.backend;

    if (store.shareClient && store.shareClient.use){
      emitter = store.shareClient;
    }

    emitter.use('connect',  handleConnect);
...
function doneInitialization(store, stream, request){
  // sharedb
  if (store.backend === store.shareClient || !store.shareClient) {
    store.backend.on('connect', function(data){
      var agent = data.agent;
      done(agent);
    });

    store.backend.listen(stream, request);
  // livedb
  } else {
    var agent = store.shareClient.listen(stream, request);
    done(agent);
  }
...
function createWebSocketStream(client, logger) {
  ...
  stream._write = function _write(chunk, encoding, callback) {
    ...
    //console.log('[racer-highway]', client.state, client.readyState);
    if (client.readyState === WebSocket.OPEN) {
  ...
  client.on('message', function onMessage(data) {
    ...
    try {
      ...
      console.warn('[racer-highway] Invalid message from client:', data);
      return;
    }
  ...
  stream.on('error', function onError(err) {
    console.error('[racer-highway] error:', err);
    client.stop();
  });
