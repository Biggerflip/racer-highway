...
function supportWebSockets(){
  ...
  // NB added belt & brances check (window.WebSocket != undefined) so when using a browser plugin
  //    like WebSocket Disabler in Firefox the page doesn't fail to load due to JS errors.
  return 'WebSocket' in window && window.WebSocket != undefined && window.WebSocket.CLOSING === 2;
}
